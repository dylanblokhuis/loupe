...
enum YAMLParser {
    ...
    static func parseDocuments(_ source: String) throws -> [JSONValue] {
        ...
        for rawLine in source.split(separator: "\n", omittingEmptySubsequences: false) {
            ...
            if let marker = content.last, marker == "|" || marker == ">" || content.hasSuffix("|-")
        ...
        return try documents.filter { !$0.isEmpty }.map { lines in
            var cursor = 0
            return try parseNode(lines, &cursor, minIndent: lines[0].indent)
        }
    }
    ...
    private static func parseSequence(_ lines: [Line], _ cursor: inout Int, indent: Int) throws -> JSONValue {
        ...
        while cursor < lines.count, lines[cursor].indent == indent, isSequenceEntry(lines[cursor].text) {
            ...
            if inline.isEmpty {
                ...
                synthetic.removeAll()
            } else if let blockStyle = blockScalarStyle(inline) {
                items.append(.string(try readBlockScalar(lines, &cursor, parentIndent: indent, style: blockStyle)))
            } else {
    ...
    private static func parseMapping(_ lines: [Line], _ cursor: inout Int, indent: Int) throws -> JSONValue {
        ...
        while cursor < lines.count, lines[cursor].indent == indent, !isSequenceEntry(lines[cursor].text) {
            ...
            } else if let style = blockScalarStyle(value) {
                object[key] = .string(try readBlockScalar(lines, &cursor, parentIndent: indent, style: style))
            } else {
    ...
    private static func blockScalarStyle(_ text: Substring) -> BlockStyle? {
        let head = text.prefix(while: { !$0.isWhitespace })
        guard let first = head.first, first == "|" || first == ">" else { return nil }
        let rest = head.dropFirst()
        guard rest.allSatisfy({ $0 == "-" || $0 == "+" || $0.isNumber }) else { return nil }
        return first == "|" ? .literal : .folded
    }

    private static func readBlockScalar(
        _ lines: [Line], _ cursor: inout Int, parentIndent: Int, style: BlockStyle
    ) throws -> String {
        ...
        while collected.last?.text.isEmpty == true { collected.removeLast() }
        guard let base = collected.first(where: { !$0.text.isEmpty })?.indent else { return "" }
    ...
    static func literal(_ raw: String) -> JSONValue {
        ...
        case "true", "True", "TRUE", "yes", "Yes", "on", "On": return .bool(true)
        case "false", "False", "FALSE", "no", "No", "off", "Off": return .bool(false)
        default: break
        }
