...
struct JSONNumber: Sendable, Equatable {
    ...
    var intValue: Int? {
        if isInteger { return Int(text) }
        // `Int(Double)` traps on NaN, infinity and anything past Int64, all of
        // which are reachable from valid JSON such as `1e400`.
...
enum JSONParseError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .trailingContent(let offset): return "Unexpected content after the JSON value at offset \(offset)"
        case .tooDeep: return "JSON nested more deeply than the parser allows"
        }
    }
}
...
struct JSONParser {
    ...
    /// Guards against a stack overflow on pathologically nested input; real
    /// Kubernetes objects nest a handful of levels deep.
    ...
    static func parse(_ data: Data) throws -> JSONValue {
        ...
        guard parser.index == parser.bytes.count else {
            throw JSONParseError.trailingContent(at: parser.index)
        }
        return value
    }
    ...
    private mutating func parseValue() throws -> JSONValue {
        guard let c = current else { throw JSONParseError.unexpectedEnd }
        guard depth < Self.maximumDepth else { throw JSONParseError.tooDeep }
        depth += 1
        defer { depth -= 1 }
        switch c {
    ...
    private mutating func parseUnicodeEscape() throws -> UnicodeScalar {
        ...
        if high >= 0xD800, high <= 0xDBFF {
            // Surrogate pair; the low half must follow immediately. If it does
            // not, rewind so the next escape is still decoded on its own.
            let rewind = index
            if current == UInt8(ascii: "\\"), index + 1 < bytes.count, bytes[index + 1] == UInt8(ascii: "u") {
                index += 2
                if let low = try? parseHex4(), low >= 0xDC00, low <= 0xDFFF {
